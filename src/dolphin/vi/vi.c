...
static inline BOOL VISetRegs(void) {
    int regIndex;
...
static inline void AdjustPosition(u16 acv) {
    ...
    HorVer.adjDispPosY = (u16)VI_MAX((s16)HorVer.dispPosY + displayOffsetV, frac);

    HorVer.adjDispSizeY =
        (u16)(HorVer.dispSizeY + VI_MIN((s16)HorVer.dispPosY + displayOffsetV - frac, 0) -
              VI_MAX((s16)HorVer.dispPosY + (s16)HorVer.dispSizeY + displayOffsetV - ((s16)acv * 2 - frac), 0));

    HorVer.adjPanPosY = (u16)(HorVer.panPosY - VI_MIN((s16)HorVer.dispPosY + displayOffsetV - frac, 0) / coeff);

    HorVer.adjPanSizeY =
        (u16)(HorVer.panSizeY + VI_MIN((s16)HorVer.dispPosY + displayOffsetV - frac, 0) / coeff -
...
static inline void setScalingRegs(u16 panSizeX, u16 dispSizeX, BOOL is3D) {
    u32 scale;
...
static void setFbbRegs(VIPositionInfo* hv, u32* tfbb, u32* bfbb, u32* rtfbb, u32* rbfbb) {
    ...
    if (hv->is3D) {
        calcFbbs(hv->rbufAddr, hv->panPosX, hv->adjPanPosY, hv->wordPerLine, hv->xfbMode, hv->adjDispPosY, rtfbb,
                 rbfbb);
    }
...
void VIConfigure(GXRenderModeObj* rm) {
    ...
    setVerticalRegs(HorVer.adjDispPosY, HorVer.adjDispSizeY, tm->equ, tm->acv, tm->prbOdd, tm->prbEven, tm->psbOdd,
