...
BOOL AIGetDMAEnableFlag(void) { return (__DSPRegs[DSP_DMA_CONTROL_LEN] & (1 << 15)) >> 15; }

void AIStartDMA(void) { __DSPRegs[DSP_DMA_CONTROL_LEN] = __DSPRegs[DSP_DMA_CONTROL_LEN] | 0x8000; }

void AIStopDMA(void) { __DSPRegs[DSP_DMA_CONTROL_LEN] = __DSPRegs[DSP_DMA_CONTROL_LEN] & ~0x8000; }

u32 AIGetDMABytesLeft(void) { return (__DSPRegs[DSP_DMA_BYTES_LEFT] & 0x7FFF) << 5; }
...
void AISetStreamPlayState(u32 playState) {
    ...
    if (AIGetStreamSampleRate() == 0 && playState == true) {
        ...
        __AIRegs[AI_CONTROL] =
            (__AIRegs[AI_CONTROL] & ~AI_CONTROL_STREAM_SAMPLE_COUNT) | AI_CONTROL_STREAM_SAMPLE_COUNT;
