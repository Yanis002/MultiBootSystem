...
namespace JGadget {
    namespace binary {
        ...
        inline u32 align_roundUp(u32 arg0, u32 uAlign) { return (arg0 + uAlign - 1) & ~(uAlign - 1); }
        ...
        template <int T> struct TParseData_aligned : public TParseData {
            TParseData_aligned(const void* pContent) : TParseData(pContent) {}
            void setRaw(const void* p) { static_cast<TParseData*>(this)->setRaw(p); }
        };
        ...
        struct TParse_header_block {
            ...
            bool parse(const void* ppData_inout, u32 a2) { return parse_next(&ppData_inout, a2); }
        ...
        template <typename T> struct TParseValue_raw_ {
            static T parse(const void* data) { return *(T*)data; }
        };

        template <typename T> struct TParseValue_endian_big_ : public TParseValue_raw_<T> {
            static T parse(const void* data) { return TParseValue_raw_::parse(data); }
        };

        template <typename T, template <class> class Parser> struct TParseValue : public Parser<T> {
