...
class JKRThread : JKRDisposer {
  public:
    ...
    void setCurrentHeap(JKRHeap* heap) { mCurrentHeap = heap ? heap : JKRHeap::getCurrentHeap(); }

  protected:
    void resume() { OSResumeThread(mThreadRecord); }
    BOOL sendMessage(OSMessage message) { return OSSendMessage(&mMessageQueue, message, OS_MESSAGE_NOBLOCK); }
    BOOL sendMessageBlock(OSMessage message) { return OSSendMessage(&mMessageQueue, message, OS_MESSAGE_BLOCK); }
    ...
    void jamMessageBlock(OSMessage message) { OSJamMessage(&mMessageQueue, message, OS_MESSAGE_BLOCK); }
